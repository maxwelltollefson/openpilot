...
class IntelligentCruiseButtonManagementInterface(IntelligentCruiseButtonManagementInterfaceBase):
  ...
  def create_canfd_mock_button_messages(self, packer, CS, CAN, send_button) -> list[CanData]:
    ...
    if self.CP.flags & HyundaiFlags.CANFD_ALT_BUTTONS:
      ...
      # The car does not register EVERY press (some synthetic SET+/SET- are dropped),
      # so at a fast cadence v_target advances while v_cruise_cluster doesn't -> the two
      # drift -> the controller keeps stepping the wrong direction. A 1.0s cadence (1
      # step/sec) gives each press time to land and be reflected in the cluster readback
      # before the next one, keeping belief and reality in sync.
      if (self.frame - self.last_button_frame) * DT_CTRL > 1.0:
        ...
        # 4 consecutive SET+ presses -> overshoot). 1.0s cadence = 1 step/sec.
